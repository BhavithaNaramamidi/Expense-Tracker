...
async def whatsapp(request: Request):
    ...
    for t in msg.split():
        ...
        if t in ["needs", "wants", "savings"]:
            parsed["main_category"] = {"needs":1,"wants":2,"savings":3}[t]
    ...
    if response == "__SAVE_ENTRY__":
        ...
        main_name = next(b["name"] for b in balances if b["name"].lower().startswith(
            "n" if main_cat==1 else "w" if main_cat==2 else "s"
        ))
